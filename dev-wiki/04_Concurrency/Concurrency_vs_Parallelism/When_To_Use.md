# When to Use - Concurrency vs Parallelism

## Use Cases
Concrete use cases with a bit of context.
-
-

## When to Use
Signals that suggest this is the right choice.
-
-

## When NOT to Use
Signals that suggest this is the wrong choice.
-
-

## Decision Tree
```mermaid
flowchart TD
  Q[Considering Concurrency vs Parallelism?] --> Q1[Do I have problem X?]
  Q1 -->|Yes| OK[Consider Concurrency vs Parallelism]
  Q1 -->|No| NO[Probably not needed]
```

## Real Scenarios
- Scenario 1: context, constraints, why Concurrency vs Parallelism is the right call.
-

